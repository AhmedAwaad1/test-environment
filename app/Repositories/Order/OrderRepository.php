...
class OrderRepository
{
    ...
    public function createOrder(array $data, $cart)
    {
        ...
        $totalAfterProdDisc = (float)($cart->total_price_after_discount ?? $subtotal);
        $couponDiscount     = (float)($cart->discount_amount ?? 0);
        $shipping           = (float)($data['shipping_price'] ?? 0);

        // Final price user pays
        $total = max(0, $totalAfterProdDisc - $couponDiscount + $shipping);
    ...
    {
        return \App\Models\Cart::where('user_id', $userId)->first();
    }
